...
class Action {
	constructor() {
		...
		this.nugetUsername =
			process.env.INPUT_NUGET_USERNAME || process.env.NUGET_USERNAME;
	}
	...
	_pushPackage(version, name) {
		...
		this._executeInProcess(
			`dotnet build --configuration ${this.configuration} ${this.projectFile} -property:Platform=${this.platform}`
		);

		this._executeInProcess(
			`dotnet pack ${
				this.includeSymbols
					? "--include-symbols -property:SymbolPackageFormat=snupkg"
					: ""
			} -property:NuspecFile=${this.nuspecFile} --no-build --configuration ${
				this.configuration
			} ${this.projectFile} -property:Platform=${this.platform} --output .`
		);
		...
		const pushCmd = `dotnet nuget push *.nupkg --source ${
				...
				!this.includeSymbols ? "--no-symbols" : ""
			}`,
	...
	_checkForUpdate() {
		...
		if (i % 2 !== 0 && matches[i - 3])
			urlPath += "/" + matches[matches.length - 3];

		const auth = `Basic ${Buffer.from(
			this.nugetUsername + ":" + this.nugetKey
		).toString("base64")}`;
